...
import com.nbp.cobblemon_smartphone.compat.refinedstorage.RefinedStorageAccess
...
object RefinedStorageAccessImpl : RefinedStorageAccess {
    ...
    // SmartphoneCompatManager's own mod-init phase, which can run before Refined Storage's
    // *own* init has populated its RefinedStorageApiProxy delegate ("API not loaded yet"
    // crash on startup). Registered lazily on first real use instead - by the time a player
    // actually triggers the action, every mod has long finished loading.
